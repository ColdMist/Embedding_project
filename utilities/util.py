...
# Authors: Brian Jones
# Modifications copyright 2018 Mirza Mohtashim Alam
# License: Apache 2.0

''' Utility functions for "Relational Learning with TensorFlow" tutorial '''
#Modified in create_tf_pair module. Created multiple negative sample against one positive sample
...
def create_tf_pairs(true_df, all_true_df, rng):
    ...
    Returns:
        ...
        contains rows [p1, p2, ..., pN], then this will contain i negative rows in the
        form [p1, n1...n2*i, p2, n2....n2*i, ..., pN, nN*i]. The original code by the author
        created 1 negative sample against each positive sample.
    '''
    ...
    for triple in true_df.itertuples(index=False):
        ...
        i=0
        #Create the number of currupt pairs according to requirement
        for i in range(5):
    ...
    result['truth_flag'] = np.tile([True, False, False, False, False, False], len(true_df))
    return result
